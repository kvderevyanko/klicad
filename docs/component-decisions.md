...
## Stage 7 corrective footprint decision record

The following selection is physical/DFM-only and **does not alter the approved
electrical baseline**. `hardware/generate_stage7_footprints.py` is the source
of the project library.

- **J4:** JST `B2B-XH-A` has a project-local footprint copied from the official
  XH/eXH geometry: 2.500-mm pitch, 1.000-mm drill, 1.70 x 2.00-mm copper.
  Mating harness remains `XHP-2` / `SXH-001T-P0.6`.
- **U1:** `Carrier:TI_TPS62133RGT_RGT0016C` now follows TI's RGT0016C example
  land/stencil drawing: 16 × 0.600 x 0.240-mm peripheral lands and a 1.680 x
  1.680-mm EP with TI's **1.550 x 1.550-mm (~85%) paste aperture**. Thermal
  vias are a later layout decision; they are not silently generated in the
  footprint.
- **L1 and Q1:** dedicated Coilcraft XFL4020 and Diodes DMP3130LQ footprints
  use their manufacturers' published recommended pad geometries.
- **F1/D3/Murata cases/resistors:** exact MPN/package envelope is documented;
  the copper lands are explicitly project IPC nominal patterns. Final
  assembler-pad confirmation is a PROCUREMENT DFM gate, not a schematic
  change.
- **Samtec SSW:** the official single-row `ssw-s.pdf` is now archived/audited:
  it sets 2.540-mm pitch and **1.040-mm drill** for SSW-115/107/104-02-G-S.
  The 1.70-mm copper land is still a PROJECT DESIGN CHOICE because Samtec's
  recommended board-layout print specifies the hole but not a copper diameter;
  retain assembler annular-ring confirmation before PCBA release.

All sources, coordinates, and remaining classifications are in
`docs/footprint-mechanical-review.md`.

## Stage 7 module-mechanical source closure

This is a footprint/mechanical update only; it does not change the approved
electrical baseline or the schematic.

| Module / interface | Current decision | Evidence and remaining boundary |
| --- | --- | --- |
| E220 universal socket | `Carrier:E220_T22D_Socket_400_900` remains the common 1x7 Samtec interface for one installed 400T22D **or** 900T22D. The template now shows fixing-hole 10/9/8 guide centres `(3.50,33.00)`, `(6.04,33.00)`, `(8.58,33.00)` mm in its pin-edge datum, but creates no contacts or NPTH there. | Official common EBYTE manual Section 3.3 plus official 900 `3D_E220-xxxT22D` STEP and `E220-900T22D-LIB`. The remaining check is a first-article mating-depth verification of the chosen Samtec socket, not a user re-measurement of the module. |
| OLED template | `Carrier:OLED_0p96_4pin_MechanicalTemplate_PENDING_DATUM` records body 26.000 x 26.000 mm; mount spacing X=21.740/Y=22.000 mm; header pin-X values GND/VCC/SCL/SDA = 9.190/11.730/14.270/16.810 mm from the left edge. | **USER-PROVIDED drawing data.** Only the header-row Y and finished mounting-hole diameter remain necessary for a registered mounting pattern; the mechanical template intentionally remains unapproved until those values are supplied. |
| ESP32 DevKit clone | Two rows at 2.540-mm pitch and 25.400-mm centre spacing remain verified. | The actual clone lacks a controlled manufacturer drawing. Request only A/B/C/D/E as defined in `footprint-mechanical-review.md`; do not infer an antenna keepout from the 28 x 51-mm envelope. |

### Stage 7.1 conservative preliminary placement (active)

The immediately preceding A/B/C/D/E request is **superseded as an active
placement blocker** and retained only as history.  Rev.1 authorises an
un-routed preliminary mechanical board using 28 x 51-mm DevKit, 21 x 36-mm
E220 and 26 x 26-mm OLED envelopes, with at least 5-mm module-to-module
clearance and larger removal/access clearance. The DevKit antenna end must be
at a carrier edge with a conservative no-components/no-routing placeholder;
DevKit USB-C and E220 SMA require direct external access. This does not
approve routing, a final outline, or a production mechanical release.

## Stage 8 — functional PCB placement, no electrical change

The approved electrical schematic remains unchanged. The current board is
generated by `hardware/generate_stage8_placement.py`, which gives every active
carrier reference one physical footprint and binds pads to the approved named
nets. `R10` and `R11` remain `DNP=YES` and deliberately have no PCB footprint;
the five schematic `PWR_FLAG` markers also have no footprint. This is an
explicit placement/BOM boundary, not a topology change.

The preferred comfortable preliminary outline is **145 x 90 mm**; **135 x
85 mm** is only the present minimum practical candidate. The earlier 160 x
100-mm rectangle is superseded as a temporary study canvas. The Stage 8 board
has no tracks, vias or zones. It reserves separate access regions for DevKit
USB-C/antenna, E220 SMA and the OLED body/mount envelope. The compact U1 power
cell is placed as input protection → CIN/U1 → L1 → COUT and remains subject to
the TI thermal/EMI routing policy.

`D2` uses `Carrier:WorldSemi_WS2812B-V5_PLACEMENT_CANDIDATE_NOT_RELEASED` only
to make the onboard carrier population and clearance visible. This is **not**
approval of a fabrication land pattern. The final WorldSemi pad-pattern audit
is a PCB-release blocker; it does not authorize a schematic change.
