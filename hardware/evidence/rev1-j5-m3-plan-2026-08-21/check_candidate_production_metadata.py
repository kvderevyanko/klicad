...
import check_candidate_sync as sync
...
def main():
    ...
    parser.add_argument("--metadata", type=Path, default=HERE / "candidate-production-metadata.json")
    ...
    board_refs = set(sync.board(HERE / "10-j5-m3-candidate.kicad_pcb"))
    sch = sync.symbols()
    errors = []
    if actual != EXPECTED:
