#!/usr/bin/env python3
"""Direct self-audit for the Q1+U3-corrected Rev.1 package.
...
EVIDENCE = HARDWARE / "evidence/u3-full-physical-audit-2026-09-15"
RELEASE = HARDWARE / "releases/rev1-q1-u3-footprint-correction-2026-09-15"

EXPECTED_SOURCE = {
    "pcb_sha256": "ef7cc377eb670115295f39cbd963e8910f2f4047cf5d740728abd09e1e4f25ea",
    ...
    "u3_footprint_sha256": "2f63828381821ca805033f3636ae06bafa95a82920a5340e76069a7b5233bad3",
}
...
EXPECTED_FAB = {
    ...
    "ESP32-E220-Carrier-Rev1-Q1-U3-Footprint-Correction.ipc356",
    "DRILL_REPORT.txt",
}
...
def gerber_flashes(text: str, reference: str) -> dict[str, dict[str, object]]:
    ...
    flashes: dict[str, dict[str, object]] = {}
    for line in text.splitlines():
        if match := re.fullmatch(r"D(\d+)\*", line):
            ...
            if current_ref == reference and current_pad is not None:
                flashes[current_pad] = {
...
def main() -> int:
    ...
    actual_source = {
        ...
        "u3_footprint_sha256": sha256(HARDWARE / "esp32-e220.pretty/TI_SN74AHCT1G125DBVR_SOT23-5.kicad_mod"),
    }
    ...
    q1_flashes = gerber_flashes(fcu, "Q1")
    expected_q1 = {
    ...
    u3_flashes = gerber_flashes(fcu, "U3")
    expected_u3 = {
        "1": {"pcb_mm": [88.05, 55.3], "net": "GND"},
        "2": {"pcb_mm": [89.0, 55.3], "net": "WS2812_DATA_3V3"},
        "3": {"pcb_mm": [89.95, 55.3], "net": "GND"},
        "4": {"pcb_mm": [89.95, 52.7], "net": "WS2812_DATA_5V"},
        "5": {"pcb_mm": [88.05, 52.7], "net": "5V_SYS"},
    }
    u3_comparable = {
        pad: {key: value for key, value in item.items() if key in {"pcb_mm", "net"}}
        for pad, item in u3_flashes.items()
    }
    if u3_comparable != expected_u3:
        errors.append(f"U3 direct F.Cu Gerber mismatch: {u3_comparable}")
    ...
    ipc356 = (fabrication / "ESP32-E220-Carrier-Rev1-Q1-U3-Footprint-Correction.ipc356").read_text()
    ipc_expected = {
    ...
    for pad in ("1", "2", "3", "4", "5"):
        # IPC-D-356 truncates leading characters of long net names in its
        # fixed-width field.  F.Cu X2 attributes above are authoritative for
        # the full net; this assertion checks the independently generated
        # physical U3 pad number and 0.60 x 1.10-mm size.
        if not re.search(rf"\s+U3\s+-{pad}\s+.*X0236Y0433", ipc356):
            errors.append(f"IPC-D-356 missing U3 pad {pad} / 0.60x1.10mm")
    ...
    manifest = f"""# Rev.1 Q1+U3 footprint-correction release manifest
...
- U3 project-local footprint SHA-256: {actual_source['u3_footprint_sha256']}
...
- Package integrity self-audit: {'PASS' if not errors else 'FAIL'}
...
"""
    ...
    (documentation / "FOOTPRINT_AUDIT_STATUS.md").write_text("""# Physical-footprint audit disposition

Status: **HOLD — NOT SAFE FOR PRODUCTION**.

- PASS: U1 `TPS62133RGT`; Q1 `DMP3130LQ-7`; U3
  `SN74AHCT1G125DBVR` (TI DBV0005A `4214839/K`, numerical board/Gerber/IPC-D-356 check).
- UNVERIFIED: all other populated MPNs named in
  `hardware/evidence/u3-full-physical-audit-2026-09-15/full-physical-footprint-audit.md`.

This archive is a self-consistent corrected fabrication snapshot, but it is
not an authorization to manufacture.  A production-safe disposition requires
retained primary manufacturer numerical land-pattern comparisons for every
UNVERIFIED populated MPN.
""")
    ...
    result = {
        "gate": "Q1-U3-CORRECTED GENERATED PACKAGE INTEGRITY SELF-AUDIT",
        ...
        "u3_f_cu_direct": u3_flashes,
        "physical_footprint_audit_disposition": "HOLD_UNVERIFIED",
