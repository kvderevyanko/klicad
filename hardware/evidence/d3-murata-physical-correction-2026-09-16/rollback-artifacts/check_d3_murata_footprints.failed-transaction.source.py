...
def check_geometry(ref: str, footprint: list[Any], embedded: bool, failures: list[str]) -> None:
    ...
    if line_bbox(footprint, "F.Fab") != expected["fab"]:
        failures.append(f"{ref}: F.Fab bbox {line_bbox(footprint, 'F.Fab')}, expected {expected['fab']}")
    if line_bbox(footprint, "F.CrtYd") != expected["courtyard"]:
        failures.append(f"{ref}: F.CrtYd bbox {line_bbox(footprint, 'F.CrtYd')}, expected {expected['courtyard']}")
