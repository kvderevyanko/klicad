...
def precise_rect(x1: float, y1: float, x2: float, y2: float,
                 layer: str, width: float) -> str:
    """Rectangle retaining four decimals for half-micron mechanical midpoints."""
    def precise_line(ax: float, ay: float, bx: float, by: float) -> str:
        return (
            f'  (fp_line (start {ax:.4f} {ay:.4f}) (end {bx:.4f} {by:.4f}) '
            f'(stroke (width {width:.3f}) (type default)) (fill none) (layer "{layer}"))\n'
        )
    return "".join((
        precise_line(x1, y1, x2, y1),
        precise_line(x2, y1, x2, y2),
        precise_line(x2, y2, x1, y2),
        precise_line(x1, y2, x1, y1),
    ))
...
def passive(name: str, description: str, body_x: float, body_y: float,
            pad_x: float, pad_y: float, pad_pitch: float,
            courtyard_x: float | None = None,
            courtyard_y: float | None = None) -> str:
    """Two-terminal project copy; package size comes from the named MPN."""
    courtyard_x = body_x / 2 + 0.5 if courtyard_x is None else courtyard_x
    courtyard_y = body_y / 2 + 0.5 if courtyard_y is None else courtyard_y
    return "".join((
        ...
        rect(-courtyard_x, -courtyard_y, courtyard_x, courtyard_y, "F.CrtYd", 0.05),
...
def smbj10ca() -> str:
    """Littelfuse SMBJ/DO-214AA manufacturer-recommended solder lands."""
    name = "Littelfuse_SMBJ10CA_DO214AA"
    return "".join((
        f'(footprint "{name}" (version 20240108) (generator "stage7")\n',
        '  (layer "F.Cu")\n',
        '  (descr "Littelfuse SMBJ10CA in DO-214AA/SMB. Manufacturer recommended solder pad layout: 2.160x2.260 mm lands, 2.740-mm inner gap, 4.900-mm centre pitch; bidirectional CA device.")\n',
        '  (attr smd)\n',
        text("reference", "REF**", 0, -2.810, "F.SilkS"),
        text("value", name, 0, 2.810, "F.Fab", 0.75),
        precise_rect(-2.2025, -1.8100, 2.2025, 1.8100, "F.Fab", 0.10),
...
def murata_grm188_standard() -> str:
    """Murata GRM18 Table 2 reflow lands for the +/-0.10-mm group."""
    return passive(
        "Murata_GRM188_1608Metric",
        "Murata GRM18 1608 metric (0603), C2/C4/C6/C7/C8. Manufacturer Table 2 reflow lands: a=0.75, b=0.70, c=0.70 mm on 1.45-mm centres.",
        1.60, 0.80, 0.70, 0.70, 1.45,
    )


def murata_grm188_c5() -> str:
    """Murata GRM188R61A106MAAL Table 2 reflow lands."""
    return passive(
        "Murata_GRM188R61A106MAAL_1608Metric",
        "Murata GRM188R61A106MAAL 1608 metric (0603), C5. Manufacturer Table 2 reflow lands: a=0.70, b=0.75, c=0.90 mm on 1.45-mm centres.",
        1.60, 0.80, 0.75, 0.90, 1.45,
    )


def murata_grm21_standard() -> str:
    """Murata GRM21 Table 2 reflow lands for the +/-0.15-mm group."""
    return passive(
        "Murata_GRM21_2012Metric",
        "Murata GRM21 2012 metric (0805), C1/C9/C10. Manufacturer Table 2 reflow lands: a=0.80, b=1.20, c=1.30 mm on 2.00-mm centres.",
        2.00, 1.25, 1.20, 1.30, 2.00,
        courtyard_x=1.650, courtyard_y=1.125,
    )


def murata_grm21_c3() -> str:
    """Murata GRM21BR61A226ME44 Table 2 reflow lands."""
    return passive(
        "Murata_GRM21BR61A226ME44_2012Metric",
        "Murata GRM21BR61A226ME44 2012 metric (0805), C3. Manufacturer Table 2 reflow lands: a=1.30, b=0.70, c=1.30 mm on 2.00-mm centres.",
